...
from idx_digest import issuer_public_snapshot_cli
...
def test_issuer_snapshot_cli_requires_explicit_publish_confirmation(tmp_path) -> None:
    result = runner.invoke(
        issuer_public_snapshot_cli.app,
        [
            "--manifest",
            str(tmp_path / "manifest.json"),
            "--start",
            "2026-07-02T14:30:00+07:00",
            "--end",
            "2026-07-02T15:30:00+07:00",
        ],
        env=_env(tmp_path),
    )
...
def test_issuer_snapshot_cli_runs_without_coverage_or_source_network(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(issuer_public_snapshot_cli, "IssuerPublicSnapshotSource", FakeSnapshotSource)
    monkeypatch.setattr(issuer_public_snapshot_cli, "SourceIngestionRunner", FakeSourceRunner)
    ...
    assert captured["require_external_id_prefix"] == "issuer-public-"
    assert captured["settings"].synapse_daily_enabled is False
    assert captured["settings"].ai_provider == "gemini"


def test_issuer_snapshot_cli_rejects_cross_day_window(tmp_path) -> None:
    result = runner.invoke(
        issuer_public_snapshot_cli.app,
        [
            "--manifest",
            str(tmp_path / "manifest.json"),
            "--start",
            "2026-07-02T23:30:00+07:00",
            "--end",
            "2026-07-03T00:30:00+07:00",
            "--confirm-publish",
        ],
        env=_env(tmp_path),
    )
