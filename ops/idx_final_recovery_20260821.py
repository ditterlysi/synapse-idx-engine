...
START = "2026-08-21T11:49:00+07:00"
END = "2026-08-21T22:47:00+07:00"
...
def main() -> None:
    ...
    checkpoint_path = Path("idx-final-recovery-checkpoint.json")
    ...
    try:
        ...
        report = {
            ...
            "publish": {
                ...
                "aiRateLimitDeferred": result.publish.ai_rate_limit_deferred,
                "errors": result.publish.errors,
            },
            "checkpointSeedCount": len(checkpoint.seen_ids),
            "durableCheckpointWritten": False,
        }
        Path("ops/idx-final-recovery-result.json").write_text(
